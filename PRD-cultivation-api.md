...
> **Status note (2026-05-21):** Archived as a design reference, not a
> spec we're building. A FastAPI/SQLite implementation of this surface
> was explored on the `Emerget-code-changes` branch and consciously
> declined — it contradicted the "single user, closed environment, no
> backend" architecture in `PRD.md` and `HANDOFF.md`. The value retained
> here is the **Resolved open questions** table below, which answers
> thirteen design questions left open in `PRD-data-model.md`. Those
> resolutions remain the working answers for the client-side data model
> in `db.js`, even though no server is built.
