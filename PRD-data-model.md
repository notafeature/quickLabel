...
1. **Sterile GrainLot lot-ID prefix.** Inoculated grain is `GL-…`.
   Sterile grain (phase=sterile) has no defined prefix yet. Options:
   reuse `GL-` and let phase disambiguate (clean for the unified-
   entity decision; awkward on a label that says "sterile RYE
   bag #03"); use a new prefix like `SG-` and accept the rename on
   inoculation (contradicts the single-entity decision); skip lot
   IDs on sterile bags entirely and assign one only at inoculation
   (breaks "how many sterile bags do I have").
2. **LC sub-lot splitting.** When a bulk LC jar fills 20 syringes,
   are those syringes one lot with one `remaining` value, or 20
   sub-lots with `01`, `02`, … suffixes? PRD-genetics-tracking §4.3
   defers this. Decision affects whether `remaining` is a single
   number or a list of per-syringe states.
3. **BulkSubstrateRecipe minimal fields.** §3.6 proposes a set;
   needs human confirmation. Specifically: is `hydration_target` a
   free string or a structured number+unit? Is `prep_method` an
   enum or free?
4. **Multi-source lineage on Batch.** SpawnToBulk fans in N GrainLots.
   If they share a genetic code and lineage, easy. If they share a
   genetic code but differ in lineage (`F1.T2` and `F1.T3`), what's
   the Batch's lineage? Options: take the lower (most conservative),
   take the highest, compose a multi-lineage string, drop lineage
   entirely on Batch. PRD §17's regulated boundary suggests Batch
   needs *a* lineage for label purposes.
5. **Disagreement on genetic code at SpawnToBulk.** Is it a hard
   error (refuse to record the event), a soft warning, or just
   allowed (Batch gets `genetic_code=null` or a multi-code marker)?
   Multi-genetic batches are uncommon but not impossible.
6. **Colonization-window expectations.** Used by the "ready for
   spawn" and "ready for fruiting" queries (§7.2). Per-genetic?
   Per-recipe? Global user default? Where is this defined?
7. **Flush numbering across multi-batch harvest days.** If two
   batches flush on the same day, flush_number is scoped to its
   Batch (so two HarvestLots can both be `flush_number=2`). Confirm
   this is the desired scoping.
8. **IngestRecord remaining material.** Some ingest types are
   plates (AP) or LC (LI). Should they carry the same `remaining`
   field as native AgarPlate / LiquidCulture, or should IngestEvent
   immediately create a derivative AgarPlate / LiquidCulture
   record? Either is defensible; clean answer affects whether
   PlateAgar can source directly from an IngestRecord (assumed yes
   in §5.1).
9. **Quarantine semantics.** When ContaminationFlag fires with
   `quarantine=true`, does the model *mutate* downstream lots'
   `lifecycle_status` to `contaminated`, or merely *expose* them as
   quarantined via the forward walk? §5.2 chose the latter, but
   confirm — and decide whether ContaminationLift on the source lot
   also lifts the derived flag on downstream lots.
10. **Recipe versioning.** If a user edits `CVG-STD` after batches
    have referenced it, do prior batches see the old or the new
    recipe? Likely the old (recipe references should be immutable
    snapshots at SpawnToBulk time), but that means SpawnToBulk has
    to copy the recipe contents onto the event, not just point at
    the registry. Confirm.
11. **Event immutability granularity.** Events are append-only. Can
    a typo in an event's `notes` be edited, or does correction
    require a NoteAttach? Either works; pick one.
12. **Photo storage.** Out of scope for this document but worth
    flagging: where do photos live? Blob storage, filesystem,
    inline base64? Layer 2 concern.
13. **Are GrainExpendableUnits and GrainLots really the same
    entity?** This is the central design call of §3.5. Confirm.
...
**(a) Entities:**
GeneticCode, IngestRecord, AgarPlate, LiquidCulture, GrainLot
(unified: sterile + inoculated phases), BulkSubstrateRecipe, Batch,
HarvestLot (with `state ∈ {wet, dried}`), plus registries:
GrainTypeRegistry, AgarFormulaRegistry, SubstrateComponentRegistry,
IngestTypeRegistry.

**(b) Events:**
IngestEvent, SterilizeGrain, PlateAgar, PlateLC, InoculateGrain,
SpawnToBulk, Harvest, Dry, BreakAndShake, ContaminationFlag,
ContaminationLift, WeightMeasurement, MoveLocation, ConsumePartial,
PhotoAttach, NoteAttach, Archive, Destroy, MarkGifted, MarkConsumed.

**(c) Open questions (most load-bearing first):**
1. Sterile-phase GrainLot lot-ID prefix.
2. LC sub-lot splitting (one lot with mL remaining, or N sub-lots).
3. BulkSubstrateRecipe minimum fields and structure.
4. Multi-source lineage resolution on Batch.
5. Genetic-code disagreement at SpawnToBulk — error, warn, allow.
6. Where colonization-window thresholds live.
7. Flush numbering scoping (per-batch confirmed?).
8. IngestRecord remaining vs. derivative-lot creation.
9. Quarantine: mutate downstream status or merely expose via walk.
10. Recipe versioning — snapshot at SpawnToBulk or live reference.
11. Event note edit policy.
12. Photo storage (deferred to Layer 2).
13. Final confirmation that GrainExpendableUnit and GrainLot are the
    same entity in two phases.
